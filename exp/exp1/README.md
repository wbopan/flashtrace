...
## 运行示例
```bash
# 默认 total 长度 1024,4096,8192，output 长度 32,256,512；每格 3 次
python exp/exp1/run_time_curve.py \
  --model qwen-8B \
  --model_path /opt/share/models/Qwen/Qwen3-8B/ \
  --cuda 0,5,7 \
  --attr_funcs IG,perturbation_all,attention_I_G,perturbation_REAGENT,ifr_all_positions,perturbation_CLP,ifr_multi_hop,attnlrp \
  --total_lengths 10,100,500 \
  --output_lengths 2,4,8 \
  --repeats 3 \
  --chunk_tokens 128 \
...
输出：
- `exp/exp1/out/time_curve_runs.jsonl`：每次运行的原始记录（attr、目标 total/output、实际长度、time、peak_mem、status）。
- `exp/exp1/out/time_curve_summary.csv`：按方法 + 目标 total/output 汇总的均值/方差。

## 注意事项
- `--total_lengths` 控制 prompt+output 总长度，`--output_lengths` 控制 output（sink）长度；prompt 长度按两者差值生成，实际 token 数会随 BPE 略有偏差。
