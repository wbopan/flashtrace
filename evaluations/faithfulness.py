...
def evaluate_attribution(testing_dict) -> None:
    ...
    if testing_dict["dataset_name"] == "math":
        for i, example in enumerate(tqdm(testing_dict["dataset"][0 : testing_dict["num_examples"]], desc = description)): 
            ...
            scores.append(faithfulness_test(testing_dict, llm_evaluator, prompt, indices_to_explain = [-2])) # [num_attrs, 3 scores]
        ...
        for i, example in enumerate(tqdm(testing_dict["dataset"][0 : testing_dict["num_examples"]], desc = description)): 
            ...
            scores.append(faithfulness_test(testing_dict, llm_evaluator, prompt, indices_to_explain = indices_to_explain, target = target)) # [num_attrs, 3 scores]

    elif testing_dict["dataset_name"] == "morehopqa":
        for i, example in enumerate(tqdm(testing_dict["dataset"][0 : testing_dict["num_examples"]], desc = description)): 
            context = ""
            question = example["question"]
            answer = example["answer"]
            context_len = len(example["context"])
            for i, item in enumerate(example["context"]):
                context += "".join(item[1])
                if i != context_len - 1:
                    context += " "
